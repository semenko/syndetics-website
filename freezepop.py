...
def main():
    """
    Freeze Syndetics and resync S3 staging/prod stores.
    """

    parser = argparse.ArgumentParser(description='The Syndetics S3/AWS management script.')
    ...
    # Special IAM user: syndetics-deploy-bot
    ...
    if args.deploy or args.freeze_only:
        ...
        if not args.no_freeze:
            print("Freezing Syndetics app ...")
            print("*** Look for errors here *** \n")
            syndetics = imp.load_source('syndetics', 'syndetics.py')
            frozen_syndetics = Freezer(syndetics.app)
            frozen_syndetics.freeze()
            print("")
        else:
        ...
        if args.deploy:
            ...
            deploy_to_s3(conn, 'syndetics_frozen', 'prod.syndetics.net', args.no_delete, args.overwrite_all)
            time.sleep(1)
